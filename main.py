...
@app.route('/')
def home():
    id_novo = str(uuid.uuid4())[:8]
    return redirect(f'/socio/{id_novo}')
...
.btn-tg { background: #0088cc; color: white; text-decoration: none; display: block; text-align: center; padding: 8px; border-radius: 5px; font-size: 12px; margin-bottom: 10px; font-weight: bold; }
...
<a href="https://t.me/botiqoption2" target="_blank" class="btn-tg">📱 ENTRAR NO CANAL TELEGRAM</a>
...
function acao(t) {
    ...
    if (t === 'ligar') {
        ...
        if(!u || !p) { alert("⚠️ Digite E-mail e Senha!"); return; }

        let ests = Array.from(document.querySelectorAll('.est:checked')).map(cb => cb.value);
        Object.assign(dados, {
            user: u, pass: p, tipo: document.getElementById('tipo').value, par: document.getElementById('par').value,
...
def loop_robo(sid, d):
    ...
    # MENSAGEM DO TELEGRAM NO MONITOR AO INICIAR
    logs_web[sid]['msg'] += "📢 Canal: t.me/botiqoption2\n"

    if not d.get('user') or not d.get('pass'):
        logs_web[sid]['msg'] += "❌ LOGIN NÃO INFORMADO\n"; return
    ...
    while sid in instancias:
        if lucro_sessao >= stop_win:
            logs_web[sid]['msg'] += f"🏆 STOP WIN: ${lucro_sessao:.2f}\n"
            instancias.pop(sid, None); break
        if lucro_sessao <= stop_loss:
            logs_web[sid]['msg'] += f"📉 STOP LOSS: ${lucro_sessao:.2f}\n"
            instancias.pop(sid, None); break
